...
# Read and display number of waveforms for the persistance plot
ser.write(b"VPC? NWF\r\n")
NR_TR = ser.readline().decode('utf-8').strip()
NR_TR = int(NR_TR.split(':',1)[-1])
print("NWF = %s\n"%(NR_TR))
...
# Calculate the values of the sample points on the x-axis
Arr_x = np.arange(0,NR_DP,1)
Arr_x.shape
Axx_x = Arr_x.transpose()
Arr_x.shape
